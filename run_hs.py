...
class CameraApp(tk.Tk):
    ...
    # Gọi lại quá trình phát hiện đối tượng trên khung hình cuối cùng
        frame, xyxys, confidences, class_ids = self.object_detection.detect_objects(self.latest_frame)
        # Thực hiện kiểm tra cảnh báo với các tham số đã lấy được
        if self.END_CHECKOUT:
            self.check_alert(xyxys, confidences, class_ids, frame)
    END_CHECKOUT = False
    def endCheckout(self):
        self.mode = "NONE"
        self.END_CHECKOUT = True
        self.end_time_label.config(text=f"Giờ Kết Thúc: {time.strftime('%H:%M:%S')}")
        list_hoc_sinh = self.nguoi_dung_dal.get()
        list_checkin = self.checkin_dal.get()
        id_chua_checkout = [
        ...
        # Bắt đầu quá trình kiểm tra cảnh báo sau khi checkout
    ...
    def start_alert_check(self):
        # Gọi hàm kiểm tra cảnh báo sau khi kết thúc checkout
        if self.END_CHECKOUT:
            self.check_alert(self.canvas_right, self.polygons_right)
    ...
    def alert(self, frame):
        ...
        threading.Thread(target=self.send_telegram_alert, args=(frame,), daemon=True).start()
    ...
    def check_alert(self, xyxys, confidences, class_ids, frame):
        ...
        if len(objects_in_polygons) > 0 and objects_in_polygons[0] == 1 and all(count == 0 for count in objects_in_polygons[1:]):
            print("Cảnh báo: Chỉ còn 1 người trong đa giác 1, phát loa cảnh báo và gửi hình ảnh về Telegram")
