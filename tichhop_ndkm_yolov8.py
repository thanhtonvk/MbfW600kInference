...
class CameraApp(tk.Tk):
    ...
    count_khong_xuong_xe = 0
    canh_bao_lien_tuc = False
    ...
    def process_video_right(self, video_path):
        ...
        while self.cap_right.isOpened():
            ...
            if ret:
                ...
                image = frame.copy()
                frame, xyxys, confidences, class_ids = (
                    self.object_detection.detect_objects(image)
                )
                ...
                if elapsed_time > 1:
                    ...
                    self.update_fps_display(self.canvas_right, round(fps))
                if self.modeYolo == "START":
                    if self.canh_bao_lien_tuc:
                        print('Canh bao lien tuc')
                        threading.Timer(
                            5, self.alert, args=[frame]
                        ).start()
                        
                    
                    check_hs = self.check_alert(xyxys, confidences, class_ids, frame)
                    
                    if check_hs:
                        self.count_khong_xuong_xe+=1
                        print("van con hoc sinh, yeu cau xuong xe")
                        
                        self.play_alert_sound()
                        if  self.count_khong_xuong_xe==5:
                            self.re_check_alert(xyxys, confidences, class_ids, frame)
                    else:
                        self.canh_bao_lien_tuc= False
                        # self.modeYolo = "END"
            else:
                break
        with self.lock_right:
    ...
    def check_alert(self, xyxys, confidences, class_ids, frame):
        ...
        for xyxy, confidence, class_id in zip(xyxys, confidences, class_ids):
            if xyxy.size == 4:
                ...
                x_center = (x1 + x2) // 2
                y_center = (y1 + y2) // 2
                if self.is_point_in_quadrilateral(x_center,y_center, self.polygons_right[0]):
                    count_detect += 1
        if count_detect > 0:
            return True
        return False
